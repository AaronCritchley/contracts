...
def test_number_of_spaces():
    ...
    assert number_of_spaces(' ') == 1
    assert number_of_spaces('  ') == 2
    assert number_of_spaces('11') == 0
    assert number_of_spaces(' 223') == 1
    assert number_of_spaces('  4343') == 2
...
def test_inline_params():
    def test_inline_parsing(docstring, expected_type="type",
        ...
        assert info.params["name"].type == expected_type
        assert info.params["name"].desc == expected_desc
...
def test_inline_returns():
    def test_inline_parsing(docstring, expected_type="type",
                            expected_desc="desc"):
        info = DocStringInfo.parse(docstring)
        assert len(info.returns) > 0
        assert info.returns[0].type == expected_type
        assert info.returns[0].desc == expected_desc
